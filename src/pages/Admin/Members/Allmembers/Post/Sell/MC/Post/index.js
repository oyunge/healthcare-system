         ...
         if(isApproved === false){
          ...
          try {
            ...
            if (userSnapshot.exists) {
              // Get the mcworks array from the user document
              const mcworks = userSnapshot.get('mcworks');
      
              // Check if the mcworks array is valid and has an image at the specified index
              if (Array.isArray(mcworks) && mcworks.length > index) {
                // Update the rate of the image at the specified index
                mcworks[index].rate = value1;
      
                // Update the user document with the updated mcworks array
                await userRef.update({ mcworks });
                ...
                toast.error('Invalid mcworks array or indexToUpdate is out of bounds',{
                  position: 'top-center'
              })
              }
            } else {
...
>
        ...
        closeButton>
            <Modal.Title>Link {index+1} - Market Connect</Modal.Title>
        </Modal.Header>
        <Modal.Body
        style={{
            background: 'linear-gradient(310deg, #2E2EFF, #81c784)',
